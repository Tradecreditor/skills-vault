...
3. Skills: for each skills/*/SKILL.md, name == folder, description present, at most 300 chars, contains "Use when"; body under 500 lines; metadata values are strings; vault_status present; the table in skills/README.md has exactly one row per skill folder and its vault_status / origin_type cells match the frontmatter (add a missing row or fix a cell as a mechanical fix). (You may run bash scripts/vault-guard-check.sh HEAD~1 HEAD Tradecreditor for the mechanical part.)
