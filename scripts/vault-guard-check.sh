...
# Rules:
#   everyone : raw/ is add-only (never modify, rename or delete) · skills/*/SKILL.md must be valid
#   non-core : may not delete or rename-away files · may not touch protected paths (.github/, this script, CLAUDE.md, AGENTS.md)
...
if [ $IS_CORE = 0 ]; then
  ...
  PROT=$(git diff --name-only -M50% "$BASE" "$HEAD" -- .github/ scripts/vault-guard-check.sh CLAUDE.md AGENTS.md || true)
  ...
  echo "core account: deletions and protected-path edits allowed (audited)"
  git diff --name-status -M50% "$BASE" "$HEAD" | awk '$1=="D" || $1 ~ /^R/ {print "  " $0}' || true
fi
...
for f in skills/*/SKILL.md; do
  ...
  [ "${#desc}" -le 1024 ] || { echo "::error file=$f::description longer than 1024 characters"; FAIL=1; }
